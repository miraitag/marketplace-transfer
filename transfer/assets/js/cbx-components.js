// willSlick(<object - array> | <object>, [ <object> options slick]  )

//@query -> [ <strong>, <array> ]  mobile | portrait | desktop | full
//@type -> dots | arrows
...
$('').willSlick({type:"arrows", query:'full'})
...
$.fn.extend({
	...
	modalCompare: function(){
		this.compare = this;
		this.button = this.compare.find('> .cbx-icon');
		this.onOpen = function(){
			var self = this;
			this.button.on('click', function(){

				if (self.compare.attr('m-open') =="true"){
					self.compare.attr('m-open','false')
				}else{
					self.compare.attr('m-open','true')
				}
			})
		};
